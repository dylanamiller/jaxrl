...
class MuZero:
    ...
    def train(self):
        ...
        self.trainer_worker = mp.shared_memory.SharedMemory(
            'trainer', create=True, size=sys.getsizeof(self.trainer)
        )
        self.shared_storage_worker = mp.shared_memory.SharedMemory(
            'trainer', create=True, size=sys.getsizeof(self.shared_storage_worker)
        )
        self.replay_buffer_worker = mp.shared_memory.SharedMemory(
            'trainer', create=True, size=sys.getsizeof(self.replay_buffer_worker)
        )
        self.self_play_worker = mp.shared_memory.SharedMemory(
            'trainer', create=True, size=sys.getsizeof(self.self_play_worker)
        )
